...
- 📚 **Library**: a collection of standalone Markdown documents with freely chosen categories (travel guides, recipes, manuals …) at `/bibliothek` — each entry optionally with a **PDF**, either uploaded or generated from the text; **cover image can be generated by AI** (Google Gemini, your own API key required); the collection name is freely configurable
...
- 📈 **Home Assistant sensors & notifications**, RSS, PWA, SEO (sitemap/robots.txt), backup, **earlier versions to restore** & static export
