...
if [ -n "$SMB_SERVER" ] && [ -n "$SMB_SHARE" ]; then
    ...
    # noserverino: FritzBox liefert instabile Inode-Nummern → ESTALE-Fix; cache=none als Absicherung
    OPTS="vers=3.0,uid=0,gid=0,file_mode=0755,dir_mode=0755,noperm,sec=ntlmssp,nodfs,iocharset=utf8,soft,noserverino,cache=none,actimeo=1"
    if [ -n "$SMB_USER" ]; then
