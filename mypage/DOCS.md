...
- **Nicht gefunden (404)** — eine Liste der Aufrufe, die ins Leere liefen, **nach Adresse gebündelt** (nicht Zeile für Zeile: der erste Scanner würde die Ablage sonst mit `/wp-login.php` füllen). Je Adresse Anzahl, Zeitpunkt des letzten Versuchs, Verweisgeber und bis zu drei Marken.
  ...
  - **„Sonde"** kennzeichnet Aufrufe, die fremde Software suchen: `/xmlrpc.php` und `/wp-*` (WordPress), `/.env`, `/.git/`, `/.ssh` (ausgeplauderte Zugangsdaten), `/asset-manifest.json` und `/api/graphql` (eine React-App mit Schnittstelle), dazu alles auf `.php`, `.asp`, `.jsp`. Nichts davon gibt es hier; reparieren lässt sich daran nichts. Solche Zeilen stehen **ganz unten** und sind über den Haken *Sonden ausblenden* voreingestellt weg.
