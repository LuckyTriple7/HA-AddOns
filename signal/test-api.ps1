# Signal Add-on API Tester
#
# Spricht den Token-Port an (Standard 17787), nicht 17777. Auf 17777
...
# "API-Token" gesetzt, Port 17787 unter Netzwerk freigegeben.
#
# Den Token nicht hier eintragen - die Datei liegt in git. Entweder vorher
#   $env:SIGNAL_API_TOKEN = "..."
# setzen oder beim Start eingeben.

$HA_IP   = if ($env:SIGNAL_HOST) { $env:SIGNAL_HOST } else { "homeassistant.local" }
$HA_PORT = if ($env:SIGNAL_API_PORT) { $env:SIGNAL_API_PORT } else { 17787 }
...
function Show-Menu {
    ...
    Write-Host "  Signal API Tester  ($BASE)" -ForegroundColor Cyan
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "  1) Status abrufen"
    Write-Host "  2) Chats auflisten"
    Write-Host "  3) Nachricht senden"
    Write-Host "  4) Letzte empfangene Nachricht"
...
function Get-Status {
    ...
    try {
        ...
        Write-Host ""
        # 'linked' heisst verbunden, alles andere ist ein Zwischenzustand
        $farbe = if ($r.status -eq "linked") { "Green" } else { "Yellow" }
        Write-Host "Status  : " -NoNewline; Write-Host $r.status -ForegroundColor $farbe
        Write-Host "Nummer  : $($r.phone)"
...
function Send-Message {
    ...
    Write-Host ""
    $nr = Read-Host "Chat-Nummer waehlen (oder leer lassen fuer manuelle Eingabe)"
    if ($nr -match '^[0-9]+$' -and [int]$nr -ge 1 -and [int]$nr -le $chats.Count) {
        $to = $chats[[int]$nr - 1].id
        Write-Host "Sende an: $($chats[[int]$nr - 1].name) ($to)" -ForegroundColor Yellow
    } else {
        # Signal adressiert nach Rufnummer im E.164-Format oder Gruppen-ID
        $to = Read-Host "Rufnummer im Format +4917612345678 oder Gruppen-ID"
    }
...
function Get-LastReceived {
    Write-Host "Frage letzte empfangene Nachricht ab ..." -ForegroundColor DarkGray
    try {
        $r = Invoke-RestMethod -Uri "$BASE/api/last-received" -Method GET -Headers $HEADERS -TimeoutSec $TIMEOUT
        if (-not $r) {
            Write-Host "Noch nichts empfangen." -ForegroundColor Yellow
            return
        }
        Write-Host ""
        Write-Host "Zeitpunkt : $($r.iso)"
        Write-Host "Chat      : $($r.chatName)"
        Write-Host "Von       : $($r.contact)"
        Write-Host "Vorschau  : $($r.preview)"
    } catch {
        Show-Fehler $_
    }
}
...
while ($true) {
    ...
    switch ($choice) {
        ...
        "3" { Send-Message }
        "4" { Get-LastReceived }
        "5" { Write-Host "Tschuess!" -ForegroundColor Cyan }
