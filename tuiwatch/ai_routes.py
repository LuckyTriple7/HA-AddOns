...
_AI_SECTIONS = (
    ...
    "- Wind: für jedes Hotel einzeln einordnen (Reisemonat, ortsgenau "
    "recherchiert) — keine allgemeinen Regionsangaben („in der Region weht oft "
    "Wind“), sondern explizit pro Hotel/Ort. Wenn ein belastbarer Zahlenwert "
    "auffindbar ist, nenne ihn (km/h oder Beaufort); wenn nicht, ordne den Ort "
    "kurz qualitativ ein (z. B. „meist ruhig“, „regelmäßig böig“, "
    "„Nachmittagsbrise“) — keine Zahl erfinden, aber auch nicht auf die fehlende "
    "Zahl herumreiten. Vergleiche in jedem Fall die Orte direkt: welches Hotel "
    "ist spürbar windiger/ruhiger als die anderen\n"
)
...
_DEFAULT_COMPARE_INSTRUCTIONS = (
    ...
    "Wind als eigene Zeile — Zahlenwert je Hotel, wenn belastbar auffindbar, sonst "
    "eine kurze qualitative Einordnung wie „meist ruhig“/„regelmäßig böig“) und "
    "einer klaren Empfehlung, welches Hotel für wen (z. B. Familie, Paar, Party, Ruhe) "
    "am besten passt. In den Tabellenzellen steht nur die Einschätzung selbst — keine "
    "Hinweise zur Quellenlage („kein belastbarer Wert auffindbar“, „nicht sauber "
    "belegbar“); solche Vorbehalte gehören höchstens in einen kurzen Satz im Fließtext "
    "unter der Tabelle. Schreibe auf Deutsch, sprich den Nutzer dabei durchgehend mit "
    ...
    "auffindbar ist, sag das einmal kurz statt zu spekulieren — nicht in jeder Zeile "
    "wiederholen. Gib direkt die fertige Antwort "
...
_AI_PRICING = {  # USD pro 1 Mio Tokens (Input/Output) — Anthropic-Listenpreise,
    ...
    # Gemini-Listenpreise (Google AI, Stand September 2026). Die 3.6-/3.7-/3.8-Flash
    # kosten bis 31.12.2026 befristet die Hälfte — hier stehen die regulären Preise.
    # 3.5-flash und 2.5-flash sind aus der Auswahl (siehe A._MODEL_SUCCESSOR) und
    # bleiben nur für alte Verlaufseinträge stehen.
