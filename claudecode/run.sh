...
if [ -f "$NPM_GLOBAL_DIR/bin/claude" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Using npm-updated Claude Code: $(claude --version 2>/dev/null)"
fi
...
# Log configuration
echo "[INFO] [$(date +%H:%M:%S)] Configuration:"
echo "[INFO] [$(date +%H:%M:%S)] model                  : $MODEL"
echo "[INFO] [$(date +%H:%M:%S)] enable_mcp             : $ENABLE_MCP"
echo "[INFO] [$(date +%H:%M:%S)] enable_playwright_mcp  : $ENABLE_PLAYWRIGHT"
echo "[INFO] [$(date +%H:%M:%S)] playwright_cdp_host    : ${PLAYWRIGHT_HOST:-auto-detect}"
echo "[INFO] [$(date +%H:%M:%S)] terminal_font_size     : $FONT_SIZE"
echo "[INFO] [$(date +%H:%M:%S)] terminal_theme         : $THEME"
echo "[INFO] [$(date +%H:%M:%S)] session_persistence    : $SESSION_PERSIST"
echo "[INFO] [$(date +%H:%M:%S)] claude_autostart       : $CLAUDE_AUTOSTART"
echo "[INFO] [$(date +%H:%M:%S)] auto_update_claude     : $AUTO_UPDATE"
echo "[INFO] [$(date +%H:%M:%S)] notify_on_update       : $NOTIFY_ON_UPDATE"
echo "[INFO] [$(date +%H:%M:%S)] export_memory          : $EXPORT_MEMORY"
echo "[INFO] [$(date +%H:%M:%S)] export_memory_interval : ${EXPORT_MEMORY_INTERVAL} min"

# Auto-detect Playwright Browser hostname if not explicitly set
if [ -z "$PLAYWRIGHT_HOST" ] && [ "$ENABLE_PLAYWRIGHT" = "true" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Auto-detecting Playwright Browser hostname..."
    ...
    if [ -n "$PLAYWRIGHT_HOST" ] && [ "$PLAYWRIGHT_HOST" != "null" ]; then
        echo "[INFO] [$(date +%H:%M:%S)] Found Playwright Browser: $PLAYWRIGHT_HOST"
    else
        echo "[WARN] [$(date +%H:%M:%S)] Playwright Browser add-on not found, using default hostname"
...
if [ "$AUTO_UPDATE" = "true" ]; then
    ...
    if [ -n "$LATEST_VER" ] && [ -n "$CURRENT_VER" ] && [ "$CURRENT_VER" != "$LATEST_VER" ]; then
        echo "[INFO] [$(date +%H:%M:%S)] Updating Claude Code from $CURRENT_VER to $LATEST_VER..."
        ...
        echo "[INFO] [$(date +%H:%M:%S)] Claude Code update complete: $(claude --version 2>/dev/null)"
    else
        echo "[INFO] [$(date +%H:%M:%S)] Claude Code $CURRENT_VER is up to date"
    fi
fi
...
do_memory_export() {
    ...
    if [ -d "$MEMORY_SRC" ]; then
        ...
        echo "[INFO] [$(date +%H:%M:%S)] Memory exported: $(ls /config/memory/*.md 2>/dev/null | wc -l) file(s) → /config/memory/"
    else
        echo "[INFO] [$(date +%H:%M:%S)] No memory directory found at $MEMORY_SRC — skipping"
    fi
    ...
    if [ -d "$COMMANDS_SRC" ] && [ -n "$(ls -A "$COMMANDS_SRC" 2>/dev/null)" ]; then
        ...
        echo "[INFO] [$(date +%H:%M:%S)] Commands exported: $(ls /config/commands/ 2>/dev/null | wc -l) file(s) → /config/commands/"
    else
        echo "[INFO] [$(date +%H:%M:%S)] No custom commands found — skipping"
    fi
}

if [ "$EXPORT_MEMORY" = "true" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Exporting Claude memory and commands to /config/..."
    do_memory_export
    if [ "$EXPORT_MEMORY_INTERVAL" -gt 0 ] 2>/dev/null; then
        echo "[INFO] [$(date +%H:%M:%S)] Memory backup started (interval: ${EXPORT_MEMORY_INTERVAL} min)"
        (while true; do
            sleep $(( EXPORT_MEMORY_INTERVAL * 60 ))
            echo "[INFO] [$(date +%H:%M:%S)] Running scheduled memory backup..."
            do_memory_export
        done) &
    fi
else
    echo "[INFO] [$(date +%H:%M:%S)] Memory export disabled"
fi
...
if [ "$ENABLE_MCP" = "true" ]; then
    ...
    jq --arg token "$SUPERVISOR_TOKEN" \
        ...
        "$SETTINGS_FILE" > /tmp/settings.tmp && mv /tmp/settings.tmp "$SETTINGS_FILE"
    echo "[INFO] [$(date +%H:%M:%S)] MCP configured with Home Assistant integration"
    echo "[INFO] [$(date +%H:%M:%S)] Pre-authorized read-only MCP tools"
else
    echo "[INFO] [$(date +%H:%M:%S)] MCP disabled"
fi
...
if [ "$ENABLE_PLAYWRIGHT" = "true" ]; then
    ...
    if getent hosts "$PLAYWRIGHT_HOST" > /dev/null 2>&1; then
        ...
        echo "[INFO] [$(date +%H:%M:%S)] socat forwarding localhost:80 → $PLAYWRIGHT_HOST:9222"
    fi
    claude mcp add-json playwright \
        '{"command":"npx","args":["--no-install","@playwright/mcp","--cdp-endpoint","http://localhost:80"]}' \
        -s user
    echo "[INFO] [$(date +%H:%M:%S)] Playwright MCP enabled (CDP: http://localhost:80 → ${PLAYWRIGHT_HOST}:9222)"
    echo "[INFO] [$(date +%H:%M:%S)] Make sure the Playwright Browser add-on is installed and running"
else
    echo "[INFO] [$(date +%H:%M:%S)] Playwright MCP disabled"
fi
...
if [ "$AUTO_UPDATE" = "true" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Update checker started (interval: 1h)"
    (while true; do
        sleep 3600
        echo "[INFO] [$(date +%H:%M:%S)] Checking for Claude Code updates..."
        ...
        if [ -n "$LV" ] && [ -n "$IV" ] && [ "$IV" != "$LV" ]; then
            ...
            echo "[INFO] [$(date +%H:%M:%S)] Update available: $LV (installed: $IV)"
            ...
            echo "[INFO] [$(date +%H:%M:%S)] Claude Code $IV is up to date"
            if [ "$NOTIFY_ON_UPDATE" = "true" ]; then
    ...
    echo "[INFO] [$(date +%H:%M:%S)] Auto-update disabled — update checker not started"
fi
