...
echo "[INFO] [$(date +%H:%M:%S)] Prüfe HA Token ..."
if [ -z "$HA_TOKEN" ]; then
    echo "[WARN] [$(date +%H:%M:%S)] ha_token ist nicht konfiguriert — bitte in den Add-on Optionen eintragen!"
else
    ...
    if [ "$HTTP_STATUS" = "200" ]; then
        echo "[OK] [$(date +%H:%M:%S)] HA Token ist gültig (${HA_URL})"
    elif [ "$HTTP_STATUS" = "401" ]; then
        echo "[WARN] [$(date +%H:%M:%S)] HA Token ungültig oder abgelaufen — HTTP 401"
    elif [ "$HTTP_STATUS" = "000" ]; then
        echo "[WARN] [$(date +%H:%M:%S)] HA nicht erreichbar unter ${HA_URL} — Timeout oder Verbindungsfehler"
    else
        echo "[WARN] [$(date +%H:%M:%S)] HA Token-Prüfung: unerwarteter HTTP-Status ${HTTP_STATUS}"
    fi
fi
...
if [ ! -f "$USERS_FILE" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Keine users.yaml gefunden — Demo-Konfiguration wird angelegt ..."
    cat > "$USERS_FILE" <<'YAML'
  ...
  - username: demo
    ...
    templates:
      ...
      - file: card3.j2
        title: Status
YAML
    echo "[INFO] [$(date +%H:%M:%S)] users.yaml angelegt unter ${USERS_FILE}"
fi

DEMO_DIR=/config/addons_config/cardboard/demo
if [ ! -d "$DEMO_DIR" ]; then
    echo "[INFO] [$(date +%H:%M:%S)] Demo-Templates werden angelegt ..."
    mkdir -p "$DEMO_DIR"
    ...
    echo "[INFO] [$(date +%H:%M:%S)] Demo-Templates angelegt unter ${DEMO_DIR}"
fi

echo "[INFO] [$(date +%H:%M:%S)] Starte CardBoard — Web: ${PORT}  Admin-API: ${ADMIN_PORT} ..."
