...
if [ ! -f "$RCLONE_CONF" ]; then
    echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] Keine rclone.conf gefunden — überspringe."
    exit 0
fi
...
if [ -z "$REMOTES" ]; then
    echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] Keine Remotes in rclone.conf — überspringe."
    exit 0
fi

# Warte bis DNS verfügbar ist (bis zu 30 Sekunden)
echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] Warte auf DNS..."
DNS_OK=false
for i in $(seq 1 15); do
    ...
    || getent hosts graph.microsoft.de >/dev/null 2>&1; then
        DNS_OK=true
        echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] DNS verfügbar (nach ${i}x2s)"
        break
    fi
    sleep 2
done
...
if [ "$DNS_OK" = false ]; then
    echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] WARNUNG: DNS nach 30s nicht erreichbar — versuche trotzdem..."
fi

PORT=$BASE_PORT
for REMOTE in $REMOTES; do
    echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] Starte WebDAV-Server für ${REMOTE}: auf Port ${PORT}"
    ...
    if [ "$STARTED" = true ]; then
        echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] ${REMOTE}: WebDAV-Server läuft auf Port ${PORT}"
        ...
        echo "[rclone] [$(date '+%Y-%m-%d %H:%M:%S')] FEHLER: WebDAV für ${REMOTE}: konnte nicht gestartet werden${ERR:+ — $ERR}"
    fi
