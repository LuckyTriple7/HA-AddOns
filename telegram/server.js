...
/* ── Emoji picker ── */
#emoji-picker { display: none; flex-direction: column; position: absolute; bottom: 100%; left: 0; right: 0; padding: 8px 10px; height: 280px; z-index: 20; }
...
#emoji-search { width: 100%; box-sizing: border-box; padding: 6px 10px; margin-bottom: 6px; border: none; border-radius: 8px; font-size: 14px; outline: none; flex: 0 0 auto; }
html.dark #emoji-search { background: #1A2432; color: #fff; }
html.light #emoji-search { background: #F1F1F1; color: #111; }
...
html.dark .emoji-tab.active { background: #1A2432; }
html.light .emoji-tab.active { background: #E7E7E7; }
.emoji-grid { display: flex; flex-wrap: wrap; gap: 2px; align-content: flex-start; overflow-y: auto; flex: 1 1 auto; }
.emoji-empty { font-size: 13px; padding: 12px; opacity: 0.6; }
...
function insertEmoji(emoji){const inp=document.getElementById('msg-input');const s=inp.selectionStart,e=inp.selectionEnd;inp.value=inp.value.slice(0,s)+emoji+inp.value.slice(e);inp.selectionStart=inp.selectionEnd=s+emoji.length;inp.focus();autoResize(inp);pushEmojiRecent(emoji);}
...
const API_PORT    = parseInt(process.env.API_PORT || '17788', 10);
